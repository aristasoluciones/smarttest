...
<table class="table table-striped table-bordered table-hover" id="sample_3">
    ...
    <tbody>
        ...
        <tr class="odd gradeX">
            ...
            <td><div align="center">
				<ol>
				{foreach from=$item.direcciones item=item2}
					<li>{$item2.direccion}</li>
				{/foreach}
				</ol>
			</div></td>
            <td><div align="center">{$item.telefono}</div></td>
            <td><div align="center">{$item.email}</div></td>
            <td><div align="center">{$item.funcionario}</div></td>
            <td><div align="center">
		
            <a href="javascript:void(0)" onClick="ViewReg({$item.dependenciaId})" title="Ver Detalles">
            	<img src="{$WEB_ROOT}/images/icons/view.png" border="0">
            </a>
            <a href="javascript:void(0)" onClick="EditReg({$item.dependenciaId})" title="Editar">
            	<img src="{$WEB_ROOT}/images/icons/edit.png" border="0">
            </a>
            <a href="javascript:void(0)" onClick="DeleteReg({$item.dependenciaId})" title="Eliminar">
